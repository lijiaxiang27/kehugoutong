...
<body>
	<div class="wrap js-check-wrap">
		<ul class="nav nav-tabs">
			<li class="active"><a href="<?php echo U('user/index_qy',array('id'=>5));?>"><?php echo L('ADMIN_USER_INDEX');?></a></li>
			<li><a href="<?php echo U('user/add_qy');?>">添加区域经理</a></li>
		</ul>
        <form class="well form-search" method="post" action="<?php echo U('User/index_qy',array('id'=>5));?>">
            校区:
            <input type="text" name="user_nicename" style="width: 100px;" value="<?php echo I('request.user_nicename/s','');?>" placeholder="请输入校区">
            手机号:
            <input type="text" name="user_login" style="width: 100px;" value="<?php echo I('request.user_login/s','');?>" placeholder="请输入手机号">
            <input type="submit" class="btn btn-primary" value="搜索" />
            <a class="btn btn-danger" href="<?php echo U('User/index_qy',array('id'=>5));?>">清空</a>
        </form>
			...
			<tbody>
				...
				<?php if(is_array($users)): foreach($users as $key=>$vo): ?><tr>
					...
					<td>
						...
						<?php else: ?>
							<a href='<?php echo U("user/edit_qy",array("id"=>$vo["id"]));?>'><?php echo L('EDIT');?></a> |
